...
test('historyStats accepts direct non-EUR pair history', () => {
  ...
  assert.equal(stats.start.toFixed(2), '0.79');
  assert.equal(stats.latest.toFixed(2), '0.78');
  assert.equal(stats.high.toFixed(2), '0.80');
  assert.equal(stats.low.toFixed(2), '0.78');
});
...
test('historyStats can invert direct pair history', () => {
  ...
  assert.equal(stats.latest.toFixed(2), '0.80');
});
...
test('historyStats still supports EUR cross-rate maps', () => {
  ...
  assert.equal(stats.start.toFixed(2), '0.80');
  assert.equal(stats.latest.toFixed(2), '0.75');
});
