...
test('Android templates preserve SafarMate identity in every supported locale', async () => {
  for (const folder of ['values', 'values-ar', 'values-in', 'values-ms', 'values-tr', 'values-fr', 'values-ur']) {
    ...
    assert.equal(source.includes('<string name="app_name">SafarMate</string>'), true);
    assert.equal(source.includes('<string name="title_activity_main">SafarMate</string>'), true);
  }
