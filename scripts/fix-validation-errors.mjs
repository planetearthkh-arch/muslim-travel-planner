...
const moneyPath = 'src/money.ts';
const money = readFileSync(moneyPath, 'utf8');
const broken = "const unsigned = normalized.replace(/^+/, '').trim();";
if (!money.includes(broken)) throw new Error('Expected broken money parser expression was not found');
writeFileSync(moneyPath, money.replace(broken, "const unsigned = normalized.replace(/^\\+/, '').trim();"));

writeFileSync('src/release-hardening.test.ts', `import assert from 'node:assert/strict';
